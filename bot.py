...
async def do_bulk_rename(message, uid: int, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        status_msg = await fetch_msg.edit_text(
            f"🔄 *Renaming Started!*\n"
            f"📊 Total: `{total:,}` | Processing: `{len(files):,}`\n"
            f"_(Progress bar will update shortly...)_",
            parse_mode="Markdown"
        )

        done = failed = skipped = 0
        total_reqs = len(files)
            ...
            if new_name == old_name:
                ...
                try:
                    await asyncio.wait_for(
                        asyncio.to_thread(mega_rename_file, file_path, new_path),
                        timeout=CMD_TIMEOUT
                    )
                    done += 1
                except asyncio.TimeoutError:
                    failed += 1
                    logger.warning(f"Timeout: {file_path}")
                except Exception as e:
                    failed += 1
                    logger.warning(f"Failed {file_path}: {e}")

            # EXACT 5-SECOND PROGRESS BAR UPDATE
            current_time = time.time()
            if (current_time - last_update_time >= 5.0) or (idx == total_reqs):
                pct = int((idx / total_reqs) * 100)
                bar_filled = pct // 5
