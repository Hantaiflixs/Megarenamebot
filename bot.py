...
# ==========================================
# Commands: /start, /login, /logout, /stats, /premium
# ==========================================
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "⚡ *Stable API Engine* - Anti-Block System!\n\n"
        "📌 *Commands:*\n"
        "  `/login email password` — Mega.nz login\n"
        "  `/logout` — Logout current session\n"
        "  `/suffix @name` — Smart renaming\n"
...
async def login_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("🔄 Mega Server se login ho raha hai...")
...
async def logout_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in user_sessions:
        ...
        await update.message.reply_text("✅ *Logout Successful!*", parse_mode="Markdown")
    else:
...
# ==========================================
# Core Bulk Rename (Bengali Bot Logic + Smart Progress)
# ==========================================
    ...
    try:
        ...
        for fid, node in all_nodes.items():
            ...
            if mode == "mode_files" and t == 0:
                files_to_process.append((fid, node))
        ...
        for fid, node in files_to_process:
            ...
            if new_name != old_name:
                rename_list.append((fid, node, new_name))
            else:
                skipped += 1
                
        # Step 3: Rename One by One (Like the Bengali Bot - Very Stable)
        done = 0
        total_reqs = len(rename_list)
        
        for idx, (fid, node, new_name) in enumerate(rename_list, start=1):
            try:
                await asyncio.to_thread(m.rename, (fid, node), new_name)
                done += 1
            except Exception as e:
                logger.error(f"Rename failed: {e}")

            # Progress Bar Update (Every 10 files to avoid flood limit)
            if idx % 10 == 0 or idx == total_reqs:
            ...
            await asyncio.sleep(0.1) # Small delay to keep Mega servers happy
...
if __name__ == "__main__":
    ...
    print("🚀 Mega Bot Backend is Running (Stable Mode)...")
